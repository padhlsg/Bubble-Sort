(function () {
  ...
  async function swapBars(i, j) {
    ...
    // Check if we need to remove comparing to avoid color conflict, though CSS order handles it usually
    // Letting 'swapped' take precedence via CSS is standard.
  ...
  async function bubbleSortVisual() {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // Last element in this pass is sorted
      if (bars()[n - 1 - i]) {
        bars()[n - 1 - i].classList.add('sorted');
      }
      if (!swapped) {
        // If no swaps, the rest are sorted
